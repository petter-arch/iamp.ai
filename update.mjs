...
async function updateNews() {
  ...
  for(let i=0;i<details.length;i+=20){
    ...
    for(const item of result.videos)if(item.relevant){report.selection.relevant++;const v=batch.find(v=>v.id===item.id);if(!supportedAIQuote(item.aiEvidence,v.snippet.title,v.snippet.description)){report.rejected.push({video:v.id,title:v.snippet.title,stage:'classification-evidence',quote:item.aiEvidence});continue;}report.selection.supported++;classified.push({...v,cat:item.cat,date:v.snippet.publishedAt,trusted:discovered.get(v.id).trusted});}
  }
  for(const v of focusedSelection(classified,policy.maxSummaries||36)) {
    ...
    try {
      ...
      if(!supportedAIQuote(a.aiEvidence,s.title,s.description)){report.rejected.push({video:v.id,title:s.title,stage:'article-evidence',quote:a.aiEvidence});continue;}
